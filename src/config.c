...
void upd8_source_parse(upd8_source_t *source, config_setting_t *setting) {
    ...
    config_setting_lookup_string(setting, name_subpath, (const char **) &(source->name));
    config_setting_lookup_string(setting, update_cmd_subpath, (const char **) &(source->update_cmd));
    config_setting_lookup_int(setting, frequency_update_hours_subpath, &(source->frequency_update_hours));
    // TODO: implement
}
...
char *upd8_source_name(upd8_source_t *source) {
    return source->name;
}
